    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.user.role === 'admin') {
          router.push('/admin')
        } else if (data.user.subscription && new Date(data.user.subscription.expiresAt) > new Date()) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
          ...
          <div className="mt-6 text-center text-sm text-gray-400">
            Não tem uma conta?{' '}
            <Link href="/registro" className="text-cyan-400 hover:text-cyan-300 font-semibold">
              Criar conta grátis
            </Link>
          </div>
        </div>
